...
namespace riscv {
// Helper: I‑tipi JALR komut kelimesi oluştur (funct3=0, opcode=JALR)
...
int main() {
  ...
  uint32_t base_addr = TEXT_START + 0x100; // test adresi
  ...
  // Test 2: JALR x0, x3, -12 (rd = x0, link atılmaz)
  cpu.reset();
  ...
  // Test 3: JALR x4, x5, 0  (hedef = rs1 + 0)
  cpu.reset();
  ...
  // Test 4: JALR ile hedef adresin en düşük biti sıfırlanmalı
  cpu.reset();
